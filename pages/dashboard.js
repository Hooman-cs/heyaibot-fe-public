...
// Required for OverviewTab subscription status colors
const STATUS_UI = {
...
export default function Dashboard() {
      ...
      if (!isAdmin) {
        fetch("/api/user/subscription")
          ...
          .then(data => {
            ...
            setSubscriptionId(data.subscription?.order_id || data.subscription?.payment_id || null);
            setGateway(data.subscription?.gateway || null);
        ...
        // Fetch bots from the chat backend using the user's ID
  ...
  const CONTENT_VIEWS = {
    ...
    // ✅ FIX: Pass `bots` instead of `apiKey={session?.user?.apiKey}`
    // UserAnalyticsTab now picks the correct bot apiKey internally with a selector UI
    "Analytics": isAdmin
      ? <AdminAnalyticsTab />
      : <UserAnalyticsTab bots={bots} />,
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
            ...
            if (link.isPrimary && link.isExternal) {
              return (
                <a key={link.id} href={link.href}
                  className="flex items-center justify-between px-4 py-3.5 mt-6 mb-4 rounded-xl font-bold bg-indigo-600 text-white shadow-md hover:bg-indigo-700 hover:-translate-y-0.5 transition-all"
                >
      ...
      {/* 3. MAIN CONTENT (pushed right of sidebar) */}
...
//   // ✅ NEW: Add these two lines right here
//   const [subscriptionId, setSubscriptionId] = useState(null);
//   const [gateway, setGateway] = useState(null);
...
//             // ✅ NEW: Save the ID and Gateway to state
//             setSubscriptionId(data.subscription?.order_id || data.subscription?.payment_id || null);
//             setGateway(data.subscription?.gateway || null);
...
//   //       // ✅ NEW: Now you can pass the state variables we just created!
//   //       subscriptionId={subscriptionId}
//   //       gateway={gateway}
//   //     />
//   //   ),
...
//     "Lead Capture": <LeadCaptureTab bots={bots} />,
//     "Chat History": <ChatHistoryTab bots={bots} />,
//     "Analytics": isAdmin ? <AdminAnalyticsTab /> : <UserAnalyticsTab bots={bots} />
//   };
...
//       <aside
//         className={`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-slate-200 shadow-2xl lg:shadow-none transform transition-transform duration-300 ease-in-out flex flex-col ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
//           }`}
//       >
//         <div className="flex justify-between items-center h-20 px-6 border-b border-slate-100">
//           <Link href="/" className="flex items-center gap-3">
...
//                 key={link.id}
//                 onClick={() => handleNavClick(link.id)}
//                 className={`w-full flex items-center justify-between px-4 py-3 rounded-xl font-bold transition-all duration-200 ${isActive
//                     ? "bg-indigo-50 text-indigo-700 shadow-sm border border-indigo-100/50"
//                     : "text-slate-600 hover:bg-slate-100 hover:text-slate-900 border border-transparent"
//                   }`}
//               >
