...
export const checkUserInterest = (userMessage) => {
  ...
  const interestPatterns = [
    ...
    'sure, please', 'okay, please',
    'yes go ahead', 'yes, go ahead',
    'start now', "let's start", 'lets start',
    'start please', 'continue please',
    'show me', 'show me please',
    'yes i want that', 'yes i want it', 'yes i want this',
    "yes i'm interested", 'yes im interested',
    'i want that', 'i want it', 'i want this',
  ];
...
export const extractMainTopic = (aiResponse) => {
  ...
  const source = (tempCleaned !== firstSentence ? tempCleaned : cleaned !== firstSentence ? cleaned : firstSentence).trim();
  const word   = source.split(/\s+/)[0];
  return word?.length > 50 ? word.substring(0, 50) + '...' : word || null;
};
...
export const generateWelcomeMessage = (websiteName, backendWelcomeMessages, systemPromptContents) => {
  if (backendWelcomeMessages && backendWelcomeMessages.length > 0) {
    return backendWelcomeMessages[Math.floor(Math.random() * backendWelcomeMessages.length)];
  }
  if (systemPromptContents && systemPromptContents.length > 0) {
    const filtered = systemPromptContents.filter(Boolean);
